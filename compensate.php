...
//die;
    ...
    send_slack_message($c_id = 'hr', $string, $hr, $day);

    if ($string4 != "") {
        $hr4 = "hrfile4";
        send_slack_message($c_id = 'hr', $string4, $hr4, $day);
    }
    if ($string1 != "") {
        $hr1 = "hrfile1";
       send_slack_message($c_id = 'hr', $string1, $hr1, $day);
    }
   send_slack_message($c_id = 'hr', $string2, $hr2);
    if ($string3 != "") {
        $hr3 = "hrfile3";
        send_slack_message($c_id = 'hr', $string3, $hr3, $day);
    }
    if ($string5 != "") {
        $hr5 = "hrfile5";
         send_slack_message($c_id = 'hr', $string5, $hr5, $day);
    }
    ...
    $url = "https://slack.com/api/im.list?token=xoxp-15636967698-15636967730-56513350243-f70365b66f";
    $cid_array = array();
    $ch = curl_init();
...
//                    echo $f . "--" . $e . "--" . $c_id . "<br>";
                        ...
                        if ($d1 == "00:00") {
                            ...
                            send_slack_message($c_id, $msg);
                        }
                        if (strtotime($d1) > strtotime('10:30')) {
                            $msg = $msg."Today's Entry Time :" . $d1;
                            $hr6 = "hrfile6";
                            send_slack_message($c_id, $msg, $hr6);
                        } if ($d1 != "00:00" && strtotime($d1) < strtotime('10:30')) {
                            $msg = $msg."Today's Entry Time :" . $d1;
                            send_slack_message($c_id, $msg);
                        }
                      //  echo $msg;
...
function get_time_array($date, $link, $hr = false) {
    ...
    if (sizeof($final) == 0 && $hr == "hrfile") {
        ...
        $dayz = date('l', strtotime($date . ' -1 day'));
        $final = get_time_array($dat, $link);
        $final['day'] = $dayz;
    }
...
function send_slack_message($channelid, $sir = false, $s = false, $day = false) {
    ...
    $url = "https://slack.com/api/chat.postMessage?token=xoxp-15636967698-15636967730-56513350243-f70365b66f&attachments=" . urlencode($message) . "&channel=" . $room;
