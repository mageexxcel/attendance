...
//--- Compensation time slack notification--------------
    if (isset($_GET['pending'])) {
        ...
        $last_five_days = array_slice($list, -5);
        array_pop($set);
        //array_pop($set);
//---- end-----------
        foreach ($arr as $kk => $vv) {
            ...
            if ($to_compensate >= 10) {
                foreach ($fresult['members'] as $foo) {
                    if ($key == $foo['profile']['email'] && $key != "") {
                        ...
                        foreach ($last_five_days as $vd) {
                            $md = date("m-d-Y", strtotime($current_date));
                            if ($md == $vd) {
                                $msg = $msg . "Compensate your time before end of month of it will result in half day deduction \n";
                            }
                        }
